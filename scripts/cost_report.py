...
def generate_text_report(report_data: dict) -> str:
    ...
    output.append(
        f"Current EPS Capacity:         {format_number(summary['current_eps_capacity'])} EPS"
    )
    output.append(f"Current Tier Annual Cost:     {format_currency(summary['current_tier_cost'])}")
    output.append(
        f"Infrastructure Cost:          {format_currency(summary['infrastructure_cost'])}"
    )
    ...
    for tier, data in tiers.items():
        ...
        output.append(
            f"{tier:15} {data['max_eps']:>10,} EPS   ${data['annual_cost']:>12,.2f}/yr  {data['description']}{marker}"
        )
    output.append("")
...
def generate_markdown_report(report_data: dict) -> str:
    ...
    for category, data in infra.items():
        output.append(
            f"| {category} | {format_currency(data['cost_usd'])} | {data['description']} |"
        )
    output.append("")
    ...
    for tier, data in tiers.items():
        marker = " ✅ *Current*" if tier == summary["current_tier"].upper() else ""
        output.append(
            f"| {tier} | {format_number(data['max_eps'])} | {format_currency(data['annual_cost'])} | {data['description']}{marker} |"
        )
    output.append("")
...
def print_colored_report(report_data: dict):
    ...
    print(
        f"{Colors.GREEN}Current QRadar Tier:          {Colors.BOLD}{summary['current_tier'].upper()}{Colors.ENDC}"
    )
    ...
    print(
        f"{Colors.GREEN}EPS Reduction:                 {Colors.BOLD}{perf['avg_eps_reduction_percent']}%{Colors.ENDC}"
    )
    ...
    print(
        f"{Colors.GREEN}{Colors.BOLD}Annual Savings:               {format_currency(savings['annual'])}{Colors.ENDC}"
    )
    ...
    print(
        f"{roi_color}ROI Percentage:               {Colors.BOLD}{roi['roi_percentage']}%{Colors.ENDC}"
    )
    ...
    print(
        f"{net_color}Net Savings (after infra):    {format_currency(roi['net_savings'])}{Colors.ENDC}"
    )
    ...
    print(
        f"{Colors.GREEN}Cost Reduction Percentage:    {eff['cost_reduction_percentage']}%{Colors.ENDC}"
    )
    ...
    for category, data in infra.items():
        print(
            f"{category:25} {Colors.CYAN}{format_currency(data['cost_usd']):>15}{Colors.ENDC}   {data['description']}"
        )
    print(
        f"{'TOTAL':25} {Colors.GREEN}{Colors.BOLD}{format_currency(TOTAL_INFRASTRUCTURE_COST):>15}{Colors.ENDC}"
    )
    ...
    for tier, data in tiers.items():
        marker = (
            f" {Colors.GREEN}✅ <-- CURRENT{Colors.ENDC}"
            if tier == summary["current_tier"].upper()
            else ""
        )
        print(
            f"{tier:15} {data['max_eps']:>10,} EPS   {Colors.CYAN}{format_currency(data['annual_cost']):>12}{Colors.ENDC}/yr  {data['description']}{marker}"
        )
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Generate cost savings reports for AI Log Filter")
    parser.add_argument(
        "--format",
    ...
    parser.add_argument(
        "--output", type=str, help="Output file path (if not specified, prints to stdout)"
    )
    parser.add_argument(
        "--qradar-eps",
        type=int,
