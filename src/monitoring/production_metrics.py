...
EPS_INGESTED = Gauge(
    "ai_filter_eps_ingested", "Events per second ingested from all sources", ["source_type"]
)

EPS_TO_QRADAR = Gauge(
    "ai_filter_eps_to_qradar", "Events per second forwarded to QRadar", ["category"]
)

EPS_FILTERED = Gauge(
    "ai_filter_eps_filtered", "Events per second filtered (not sent to QRadar)", ["category"]
)

EPS_REDUCTION_RATIO = Gauge(
    "ai_filter_eps_reduction_ratio", "Ratio of logs filtered vs total (0-1)"
)
...
CRITICAL_RECALL = Gauge(
    "ai_filter_critical_recall", "Proportion of true critical events correctly identified (0-1)"
)
...
MODEL_LOAD_TIME = Gauge(
    "ai_filter_model_load_time_seconds", "Time taken to load the model", ["model"]
)
...
MODEL_VERSION = Info("ai_filter_model_version", "Current model version information")
...
FAIL_OPEN_EVENTS = Counter(
    "ai_filter_fail_open_events_total", "Number of logs forwarded due to fail-open mode", ["reason"]
)
...
ROUTING_QUEUE_SIZE = Gauge(
    "ai_filter_routing_queue_size", "Current size of routing queues", ["category"]
)

ROUTING_ERRORS = Counter(
    "ai_filter_routing_errors_total", "Errors during log routing", ["destination", "error_type"]
)
...
KAFKA_CONSUMER_OFFSET = Gauge(
    "ai_filter_kafka_consumer_offset", "Current consumer offset", ["topic", "partition"]
)

MEMORY_USAGE_BYTES = Gauge(
    "ai_filter_memory_usage_bytes", "Current memory usage in bytes", ["type"]
)
...
class ProductionMetricsCollector:
    ...
    def record_log_processed(
        ...
        # Throughput
        LOGS_PROCESSED_TOTAL.labels(source=source, category=category, model=model).inc()
        ...
        if forwarded_to_qradar:
            ...
            EPS_TO_QRADAR.labels(category=category).set(self.eps_to_qradar.get_eps())
        else:
            self.eps_filtered.add_sample(1)
            LOGS_FILTERED_TOTAL.labels(category=category, reason="classification").inc()

        # Update EPS gauges
        EPS_INGESTED.labels(source_type=source).set(self.eps_ingested.get_eps())

        # Confidence distribution
        CLASSIFICATION_CONFIDENCE.labels(category=category, model=model).observe(confidence)

        # Latency
        END_TO_END_LATENCY.observe(latency_seconds)

        # Model predictions
        MODEL_PREDICTION_COUNT.labels(model=model, category=category).inc()
    ...
    def record_batch_processed(self, batch_size: int, model: str, latency_seconds: float):
        ...
        CLASSIFICATION_LATENCY.labels(model=model, batch_size_bucket=bucket).observe(
            latency_seconds
        )

        BATCH_SIZE_ACTUAL.observe(batch_size)

    def record_false_negative(self, original_category: str, offense_type: str, severity: str):
        """Record a false negative (missed detection)."""
        FALSE_NEGATIVES_TOTAL.labels(
            original_category=original_category, offense_type=offense_type, severity=severity
        ).inc()
    ...
    def record_false_positive(self, predicted_category: str, source: str):
        ...
        FALSE_POSITIVES_TOTAL.labels(predicted_category=predicted_category, source=source).inc()
    ...
    def record_circuit_breaker_state(self, circuit_name: str, state: str, failure_count: int = 0):
    ...
    def record_model_drift(self, model: str, drift_type: str, score: float):
        """Record model drift detection."""
        MODEL_DRIFT_SCORE.labels(model=model, drift_type=drift_type).set(score)

        if score > 0.1:  # Threshold for warning
            logger.warning(
                "Model drift detected",
                extra={"model": model, "drift_type": drift_type, "score": score},
            )

    def record_routing_decision(self, category: str, destination: str):
        """Record a routing decision."""
        ROUTING_DECISIONS.labels(category=category, destination=destination).inc()

    def record_routing_error(self, destination: str, error_type: str):
        """Record a routing error."""
        ROUTING_ERRORS.labels(destination=destination, error_type=error_type).inc()
